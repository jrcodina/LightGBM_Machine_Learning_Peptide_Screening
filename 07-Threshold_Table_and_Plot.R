...
data2 <- read.csv("./data/Docking_scores/prop_DENV_Tetra.csv")
data3 <- read.csv("./data/Docking_scores/prop_WNV_Tetra.csv")
data4 <- read.csv("./data/Docking_scores/prop_ZIKV_Tetra.csv")
...
model1 <- readRDS.lgb.Booster("./Results/LightGBM_MODELS/model_CHIKV.RDs")
model2 <- readRDS.lgb.Booster("./Results/LightGBM_MODELS/model_DENV.RDs")
model3 <- readRDS.lgb.Booster("./Results/LightGBM_MODELS/model_WNV.RDs")
model4 <- readRDS.lgb.Booster("./Results/LightGBM_MODELS/model_ZIKV.RDs")
...
write.csv(cmZIKV, "./Results/ConfussionMatrix/cmZIKV.csv", row.names = F)
write.csv(cmDENV, "./Results/ConfussionMatrix/cmDENV.csv", row.names = F)
write.csv(cmCHIKV, "./Results/ConfussionMatrix/cmCHIKV.csv", row.names = F)
write.csv(cmWNV, "./Results/ConfussionMatrix/cmWNV.csv", row.names = F)
...
for (i in 1:length(cmTables)) {
  ...
  v <- letters[i]
  ggsave(plot = p, device = "png", paste0("./Results/Figures/Figure_4.1.",v, ".png"),width = 20, height = 17.5, dpi = 600)
}
