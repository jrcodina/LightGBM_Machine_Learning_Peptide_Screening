...
names(ROC) <- c("CHIKV", "DENV", "WNV", "ZIKV")

saveRDS(ROC, "./Results/ROCs/ROC-AUC_list.RDs")

names(models) <- c("ZIKV1", "ZIKV2", "ZIKV3", "DENV", "CHIKV", "WNV")
saveRDS.lgb.Booster(object = models[[1]], file = "./Results/LightGBM_MODELS/model_CHIKV.RDs")
saveRDS.lgb.Booster(object = models[[2]], file = "./Results/LightGBM_MODELS/model_DENV.RDs")
saveRDS.lgb.Booster(object = models[[3]], file = "./Results/LightGBM_MODELS/model_WNV.RDs")
saveRDS.lgb.Booster(object = models[[4]], file = "./Results/LightGBM_MODELS/model_ZIKV.RDs")

# Plots 
ROC <- readRDS("./Results/ROCs/ROC-AUC_list.RDs")
names(ROC) <- c("ZIKV", "ZIKV3", "DENV", "CHIKV", "WNV")
ROC <- ROC[c(4,3,5,1)]
...
png(filename = "./Results/Figures/Figure_3.a.png", width = 3500, height = 3200, res = 600)
...
thresholds <- data.frame(sapply(ROC, function(x) round(coords(x[[1]], "best", best.method = "topleft"),2)))
...
names(thresholds) <- c("CHIKV", "DENV", "WNV", "ZIKV")

thresholds_transposed <- as.data.frame(t(thresholds))
#write.csv(thresholds, "./Results/ROCs/Best_Thresholds.csv")
...
write.csv(ROC_CHIKV, "./Results/ROCs/CHIKV.csv", row.names = F)
write.csv(ROC_DENGV, "./Results/ROCs/DENV.csv",  row.names = F)
write.csv(ROC_WNV,   "./Results/ROCs/WNV.csv",   row.names = F)
write.csv(ROC_ZIKV,  "./Results/ROCs/ZIKV.csv",  row.names = F)
